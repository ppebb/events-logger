local function on_pre_player_died (e)
	...
	if e.cause and e.cause.type == "character" then --PvP death
		log("[DIED] " .. event_json["name"] .. " " .. (game.get_player(e.cause.player.index).name or "no-cause"))
	elseif (e.cause) then
		log("[DIED] " .. event_json["name"] .. " " .. (e.cause.name or "no-cause"))
	else
		log("[" .. event_json["event"] .. "] " .. event_json["name"] .. " " .. "no-cause") --e.g. poison damage
	end
	event_json["reason"] = reason
	helpers.write_file("game-events.out", helpers.table_to_json(event_json), true)
	log("[" .. event_json["event"] .. "] " .. event_json["name"] .. " " .. (game.get_player(e.cause.player.index).name or "no-cause"))
end
...
local function on_player_joined_game(e)
	local event_json = {}
	event_json["name"] = game.get_player(e.player_index)
...
local function on_research_started(event)
	...
	helpers.write_file("game-events.out", helpers.table_to_json(event_json), true)
	log("[" .. event_json["event"] .. "] " .. research_name .. " " .. (event.research.level or "no-level"))
end
